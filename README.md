...
| Tool | Description |
| :--- | :--- |
| [**ts-handler**](./ts-handler) | Filters logs by date/time range and normalizes timestamps. |
...
- **Pipeline Logic**:
  - **ts-handler**: Crucial for normalizing timestamps to a standard 12-digit format (`YYMMDDHHMMSS`).
  - **log-tree**: Can be used directly from `grep` or after filtering to visualize the structure.
  - **summarizer**: Depends on the normalization provided by `ts-handler` to correctly identify and mask dates.

**Possible Combinations:**
1. `ts-handler` (Standard filtering)
2. `ts-handler -> log-tree` (Filtered structure)
3. `log-tree` (Direct visualization from grep)
4. `log-tree -> summarizer` (Summarize patterns within a structure)
5. `ts-handler -> log-tree -> summarizer` (Full pipeline: filter, organize, and summarize)
...
grep -rH "ERROR" /path/to/logs | \
  ./ts-handler -start 2024-05-10 -end 2024-05-20 | \
